...
@Service
public class MotorSkillsReportService {

    private static final String MAZE_TRACKING   = "Labirent Takibi";
    private static final String COLOR_PAINTING  = "Renk Boyama";

    private final ChildRepository childRepository;
    private final ActivityRepository activityRepository;
    private final ActivityResultRepository activityResultRepository;

    public MotorSkillsReportService(
    ...
    public MotorSkillsReportDto generateReport(Long childId) {

        Child child = childRepository.findById(childId)
                .orElseThrow(() -> new RuntimeException("Çocuk bulunamadı. ID: " + childId));

        MotorSkillsReportDto report = new MotorSkillsReportDto();
        report.setChildId(child.getId());
        report.setChildName(child.getName());
        report.setMazeTracking(buildGameReport(child, MAZE_TRACKING));
        report.setColorPainting(buildGameReport(child, COLOR_PAINTING));
